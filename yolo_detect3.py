...
while True:
    ...
    cv2.putText(frame, f'Cart Total: INR {total_price:.2f} | {total_calories} cal', (10, 20), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,0), 2)
    y_offset = 45
    cv2.putText(frame, f'Items in Cart: {len(cart_items)} | Frame: {tracker.frame_count}', (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0,255,255), 2)

    y_offset += 30
    for item, qty in cart_items.items():
        info = item_info.get(item, {'price': 0, 'calories': 0})
        cv2.putText(frame, f'{item}: {qty} (INR {info["price"]*qty}, {info["calories"]*qty} cal)', (10, y_offset), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0,255,0), 1)
        y_offset += 20
...
print("\nFINAL SMART CART SUMMARY")
print("="*40)
if cart_items:
    for item, qty in cart_items.items():
        info = item_info.get(item, {'price': 0, 'calories': 0})
        print(f"{item.title()} x{qty}: INR {info['price']*qty} | {info['calories']*qty} cal")
    print("-" * 40)
